...
file1 = strftime("C:\\JSF PCD EU MTE Software\\Log Files\\Perf-SP-Left-%Y-%m-%d-%H%M%S.log", gmtime())
file2 = strftime("C:\\JSF PCD EU MTE Software\\Log Files\\JSON-SP-Left-%Y-%m-%d-%H%M%S.log", gmtime())
bufsize = 0
...
# lets plot once every five seconds, plotting is really expensive
                ...
                print >>f1, 'ML', int(mainLoopMeas), 'M', int(mainFilt), 'FC', int(fcFilt), 'FW', int(fwFilt), 'O1', int(ogl1Filt), 'O3', int(ogl3Filt), 'MV', int(marvellFilt), 'N4', int(niu4Filt), 'N5', int(niu5Filt)
