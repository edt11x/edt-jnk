...
foreach (split(/\n/, $colors)) {
    chomp;
    my $thisLine = $_;
    my @pieces = split(' ', $thisLine);
    # extract the name
    my $thisName = "otp_color";
    my $done = 0;
    foreach (@pieces)
    {
        if ($done == 0)
        {
            ...
            if ($_ =~ /\-/)
            {
                my @colorNums = split('-', $thisWord); # found the colors
                my $red   = $colorNums[0];
                my $green = $colorNums[1];
                my $blue  = $colorNums[2];
                printf("void %s(void) { glColor3f(%.1ff, %.1ff, %.1ff); }\n", $thisName, $red, $green, $blue);
                $done = 1;
            }
            else
            {
                $thisWord =~ tr/A-Z/a-z/;
                $thisName = $thisName . "_" . $thisWord;
            }
        }
    }
}
