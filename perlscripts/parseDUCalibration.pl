...
sub parseLine {
    my $thisLine = $_[0];
    # print "Here - $thisLine \n";
    if ((! empty($thisLine)) && ($thisLine =~ /\[([xy])\]\[(\d\d)\]=\s.*values=\( (.*)\)$/)) {
        my $chart = $1;
        my $intVal = $2;
        my @pieces = split(',', $3);
        # print "Chart - $chart, IntVal - $intVal, Pieces - \n";
        # print $_, " " for (@pieces);
        my $count = 0;
        for my $piece (@pieces) {
            (my $new = $piece) =~ s|(\d+\.?\d*)%|$1/100|eg;
            my $index = $intVal + $count++ / 50.0;
            # print "$chart, ", $index, ", ", $new, "\n";
            $xVals{$index} = $new if ($chart eq "x");
            $yVals{$index} = $new if ($chart eq "y");
        }
    }
}
