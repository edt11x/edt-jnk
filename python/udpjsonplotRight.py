...
file1 = strftime("C:\\JSF PCD EU MTE Software\\Log Files\\Perf-SP-Right-%Y-%m-%d-%H%M%S.log", gmtime())
file2 = strftime("C:\\JSF PCD EU MTE Software\\Log Files\\JSON-SP-Right-%Y-%m-%d-%H%M%S.log", gmtime())
bufsize = 0
...
# lets plot once every five seconds, plotting is really expensive
                ...
                if time() > (lastPlotTime + 2):
                    ...
                    plt.plot(ogl3AtTime,  ogl3Meas/4,    'c.', label='Cyan OGL3 Sleep')
                    # XXplt.plot(ogl3AtTime,  ogl3Filt/4,    'c+', label='Cyan OGL3 Sleep')
                ...
                print >>f1, 'ML', int(mainLoopMeas), 'M', int(mainFilt), 'FC', int(fcFilt), 'FW', int(fwFilt), 'O1', int(ogl1Filt), 'O3', int(ogl3Filt)/4, 'MV', int(marvellFilt), 'N4', int(niu4Filt), 'N5', int(niu5Filt)
